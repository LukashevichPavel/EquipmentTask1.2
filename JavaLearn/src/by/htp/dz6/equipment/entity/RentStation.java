...
public void newClient() throws IOException{
	...
	if (option.equals("yes")){
		...
		if (checkClientForExist(newClient)){
			System.out.println("Client uje suschestvuet v baze, vibrat' ego? (yes-> vibrat')");
			option=reader.readLine();
			if (option.equals("yes")){
				currentClient=newClient;
				if (checkClienForRent(currentClient)>3){System.out.println("Vi uje vzyali 3 equipment, sorry come back later after return some equipment");}
				else {
					
				}
			}
			else {System.out.println("nelzya sozdat' bolee 1 clienta s odinakovim passportom, program finish");}
...
public void takeToRent() throws IOException{ //metod kot. pomechaet equipment kak vzyatiy v arendu
	...
	if (numEquipment>getSizeBase()||(numEquipment==0)){System.out.println("Vsego v base "+getSizeBase()+" elementov");}
		...
		newClient();//Metod vizivaet vvod dannix novogo klienta ili predlagaet vibrat' suschestvutus4ego iz  bazi clientov	
		clientBase.printClientBase();	
			
			
			
			
			//inRentBase. (base[numEquipment-1]); //Dobavlyaem equipment v bazu arendovanogo
...
public int checkClienForRent(Client client){//metod kot. proveryaet kolvo vzyatogo Equipment-a 
...
public void saveToFile(String filename){
	 try(FileOutputStream fos=new FileOutputStream(filename))
  {
	  ...
	  String text = this.base[i].toString();
	  text+="\n";
